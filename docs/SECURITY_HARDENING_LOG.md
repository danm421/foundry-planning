...
## 2026-07-16 — Supply-chain guardrails + dependency vuln sweep

Motivated by the 2025 npm supply-chain wave (chalk/debug compromise, Shai-Hulud
worm, tj-actions tag hijack). Two halves: fix what's vulnerable now, and gate
what gets in from here.

**Fixes** — `npm audit fix` cleared the fixable advisories (undici under
`@vercel/blob`, `tmp`, `fast-uri`, `linkify-it`, `@babel/core`, otel `core`
via `@sentry/nextjs`, `vite` under vitest, and others). A targeted
`package.json` override pins `path-to-regexp` to `6.3.0` (the 6.x ReDoS patch
release) under `@vercel/routing-utils`, whose own latest still ships 6.1.0.
Two moderates are accepted with written rationale (see
`scripts/audit-gate.mjs` + future-work/security-hardening.md): `uuid`
<11.1.1 (exceljs/svix only call `v4()` — the advisory needs v3/v5/v6 with a
caller-supplied buffer) and `postcss` pinned inside `next` (build-time
stringify of our own CSS, not user input).

**Guardrails**

| Control | Where | What it stops |
|---|---|---|
| Dependabot w/ 7-day cooldown (21 for majors) | `.github/dependabot.yml` (root npm, `mobile/` npm, github-actions) | freshly-published compromised releases — most are unpublished within days |
| Blocking audit gate: high/critical on **prod** deps | `scripts/audit-gate.mjs` via `.github/workflows/security.yml` | known-vulnerable code reaching a deploy; allowlist requires written rationale |
| Lockfile lint (https, registry-only hosts, sha512 integrity) | `security.yml` | lockfile poisoning / off-registry resolution |
| `npm audit signatures` | `security.yml` | tampered tarballs; verifies registry signatures + provenance attestations |
| gitleaks secret scan (blocking) + `.gitleaksignore` | `security.yml` | committed credentials (full-history scan 2026-07-16: clean, 1 false positive) |
| OSV / malicious-package scan (advisory, SARIF → Security tab) | `security.yml` osv-scan job | MAL-* advisories npm audit can lag on |
| CodeQL (javascript-typescript, weekly + PR/push) | `.github/workflows/codeql.yml` | injectable/vulnerable first-party code patterns |
| All actions pinned to commit SHAs | every workflow | tag-hijack on third-party actions (tj-actions class) |
| `save-exact=true` | `.npmrc` | silent semver-range drift; updates flow through cooled-down Dependabot |
| `SECURITY.md` | repo root | gives reporters a private channel (repo is public) |

Weekly cron on both scanning workflows re-checks with no code change, so a new
advisory against existing pins surfaces within a week.

**Still open** (tracked in future-work/security-hardening.md): install-script
allowlisting (`@lavamoat/allow-scripts` — blocked on Vercel install-command
plumbing; `sharp`/`@sentry/cli`/`@swc/core`/`esbuild` legitimately need
scripts), branch protection with required checks, and the Settings-side
GitHub toggles (Dependabot alerts, secret-scanning push protection, private
vulnerability reporting) which need a valid `gh` login.
