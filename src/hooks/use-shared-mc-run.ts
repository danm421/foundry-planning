...
/**
 * Minimal plan shape the shared Monte Carlo run consumes. The solver builds
 * this (see build-solver-comparison-plan). The hook reads only `ref`, `id`,
 * `label`, `tree`, and `result.years` — formerly a subset of the retired
 * Comparison tab's richer ComparisonPlan.
 */
export interface SharedMcPlan {
  ref: ScenarioRef;
  id: string;
  label: string;
  tree: ClientData;
  result: { years: ProjectionYear[] };
}
 ...
 */
function cacheScenarioId(plan: SharedMcPlan): string | null {
