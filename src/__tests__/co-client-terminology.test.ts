...
function violations(file: string): string[] {
  ...
  return [...text.matchAll(DISPLAY)]
    ...
    .filter((hit) => {
      ...
      // Exclude if entire string is exactly the bare enum word "spouse"
      if (content.toLowerCase() === "spouse") return false;

      // For lowercase "spouse", apply additional exclusions
      if (/\bspouse\b/.test(hit)) {
        // c. No space at all in the content
        if (!content.includes(" ")) return false;

        // d. Contains special chars: -, _, $, |, =
