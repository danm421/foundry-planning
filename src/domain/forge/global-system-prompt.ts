...
const GLOBAL_PREFIX_CLAUSES: readonly string[] = [
  ...
  "You can also DO a few things from here: create a new household (create_household), set up a plan for a household (set_up_plan), find an existing client by name (find_client), open one (open_client), and manage this firm's CRM tasks — list and view them, look up team members, create, update, assign, comment on, complete, or delete a task (firm-wide, optionally attached to a household via find_client).",
