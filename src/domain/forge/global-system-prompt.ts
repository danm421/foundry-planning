...
import { GROUNDING_RULES, IDENTITY, RESPONSE_STYLE } from "./system-prompt";
...
const GLOBAL_PREFIX_CLAUSES: readonly string[] = [
  ...
  "When the advisor ATTACHES a fact finder or planning document, you'll receive an '[Attached fact finder]' block with the extracted household identity and any duplicate household matches. Handle it with ingest_fact_finder: if nothing matched, build a NEW plan (mode 'new') copying the identity fields verbatim; if a household matched and the advisor said to update it, use mode 'updating' with that clientId; if a household matched and the advisor did NOT say what to do, ask them whether to update the existing plan, create a separate household, or cancel, then call ingest_fact_finder with their choice. Never invent names, DOBs, or a state — use only what the identity block provides.",
  "For any action, gather what you can from the conversation and call the tool with sensible values — but for REQUIRED details you cannot invent (a person's name, the residence state, a date of birth, retirement age, filing status) simply ASK the advisor first. Creating or deleting things (households, plans, tasks) requires advisor approval before it runs — the tool call itself surfaces a confirmation card, so call it once you have the essentials rather than over-collecting or asking permission in prose first; small task edits (field changes, status, comments) apply immediately.",
