...
//  - Categories flagged `excludedFromBudget` (and every category under an
//    excluded group) leave `groups` entirely and land in `excluded`, so they
//    touch no budget total, no group rollup and no donut slice.

import type { LeafCell, GroupCell, ExcludedCell, BudgetSummary } from "@/lib/portal/contracts";
export type { LeafCell, GroupCell, ExcludedCell, BudgetSummary };
...
export type BudgetCategory = {
  ...
  kind: "group" | "category";
  sortOrder: number;
  /** Optional so pure-rollup callers can omit it; the DB always supplies it. */
...
export function computeBudgetSummary(input: {
  ...
  // An excluded GROUP takes its categories with it — the flag on the group is
  // the whole-group switch, so a leaf never has to be flagged one by one.
  const excludedCells: ExcludedCell[] = [];
  const groupCells: GroupCell[] = [];
  const toCell = (l: BudgetCategory): LeafCell => ({
    id: l.id,
    name: l.name,
    slug: l.slug,
    color: l.color,
  ...
  for (const g of groups) {
    const groupExcluded = g.excludedFromBudget === true;
    const leafCats = (leavesByGroup.get(g.id) ?? []).sort(
      (a, b) => a.sortOrder - b.sortOrder,
    );
    const leaves: LeafCell[] = [];
    for (const l of leafCats) {
      if (groupExcluded || l.excludedFromBudget === true) {
        excludedCells.push({ ...toCell(l), groupName: g.name });
      } else {
        leaves.push(toCell(l));
      }
    }
    // Nothing budgetable left → the group drops out rather than showing an
    // empty $0 row. (A group with no categories at all is a different thing —
    // a freshly created group — and still shows.)
    if (groupExcluded || (leafCats.length > 0 && leaves.length === 0)) continue;
  ...
  return {
    ...
    totalExcluded: round2(excludedCells.reduce((s, l) => s + l.actual, 0)),
    totalBudget,
    totalSpent,
