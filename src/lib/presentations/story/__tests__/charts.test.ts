...
describe("buildStoryCharts", () => {
  it("returns the portfolio bars the chart draws, one per projection year", () => {
    const charts = buildStoryCharts({ years: years(), estateBars: null });
  ...
  it("passes the estate bars through untouched — one array, not a second derivation", () => {
    const bars = [
      { label: "Today", netToHeirs: 1, federal: 0, state: 0, probate: 0, ird: 0, debts: 0, total: 1 },
    ];
    const charts = buildStoryCharts({ years: years(), estateBars: bars });
    expect(charts.estate).toBe(bars);
  });
});
