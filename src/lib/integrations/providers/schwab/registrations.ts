 ...
 * patterns must precede the general /ira/ catch-all. The 401(k) and 403(b)
 * patterns must also precede the bare Roth pattern: Schwab registration names
 * like "Roth Individual 401(k)" and "Roth 403(b)" contain "Roth" but are
 * employer plans, not IRAs, so the employer-plan patterns need first crack at
 * them before the broader Roth match (needed to also catch things like
 * "Roth Contributory IRA") would otherwise misroute them.
 *
