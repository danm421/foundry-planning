// src/lib/orion/households.test.ts
...
import { getHouseholdLinks, linkHousehold, unlinkHousehold } from "./households";
...
afterAll(async () => {
  // Delete clients by firmId — cascades to orionHouseholdLinks and crmHouseholds
...
describe("orion household links", () => {
  it("linkHousehold inserts a row; getHouseholdLinks returns it", async () => {
    ...
    await linkHousehold({ firmId, clientId, orionHouseholdId: "hhA", userId: "user1" });

    const rows = await getHouseholdLinks(firmId);
    const row = rows.find((r) => r.clientId === clientId);
    expect(row).toBeDefined();
    expect(row?.orionHouseholdId).toBe("hhA");
  ...
  it("linkHousehold with same clientId updates in place (onConflict)", async () => {
    ...
    await linkHousehold({ firmId, clientId, orionHouseholdId: "hhB", userId: "user1" });
    const beforeRows = await getHouseholdLinks(firmId);
    const before = beforeRows.find((r) => r.clientId === clientId);
    expect(before?.orionHouseholdId).toBe("hhB");
    const beforeUpdatedAt = before?.updatedAt;

    // Small delay so updatedAt advances
    await new Promise((resolve) => setTimeout(resolve, 10));

    await linkHousehold({ firmId, clientId, orionHouseholdId: "hhB_updated", userId: "user1" });
    const afterRows = await getHouseholdLinks(firmId);
    const matching = afterRows.filter((r) => r.clientId === clientId);
    expect(matching).toHaveLength(1);
    expect(matching[0].orionHouseholdId).toBe("hhB_updated");
  ...
  it("unlinkHousehold deletes the row; getHouseholdLinks no longer returns it", async () => {
    const { clientId } = await createTestClientWithScenario(firmId);
    await linkHousehold({ firmId, clientId, orionHouseholdId: "hhC", userId: "user2" });

    // Confirm it exists
    const before = await getHouseholdLinks(firmId);
    expect(before.some((r) => r.clientId === clientId)).toBe(true);

    await unlinkHousehold(firmId, clientId);

    const after = await getHouseholdLinks(firmId);
