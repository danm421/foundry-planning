// src/lib/orion/sync.test.ts
import { describe, it, expect, beforeAll, afterAll } from "vitest";
import { randomBytes } from "node:crypto";
import { and, eq } from "drizzle-orm";
import { db } from "@/db";
import { accounts, clientImports, clients, crmHouseholds, orionConnections } from "@/db/schema";
...
afterAll(async () => {
  for (const firmId of firmIds) {
    ...
    // but delete clients first → cascades orionHouseholdLinks + accounts + clientImports.
...
/**
 * Injected in place of the real OrionClient. Returns plain objects matching the
 * mapper's reads. Uses an UNTICKERED position so resolveHoldingsForCommit
 * short-circuits with zero tickers (no network / classification calls).
 */
...
describe("syncFirm", () => {
  it("EXACT match auto-commits in place and is idempotent", async () => {
    ...
    await upsertConnection({ firmId, accessToken: "fake", userId: "u1" });
    ...
    await linkHousehold({ firmId, clientId, orionHouseholdId: "hh1", userId: "u1" });
    ...
    const second = await syncFirm(firmId, { trigger: "manual", clientId, client, userId: "u1" });
  ...
  it("NEW account lands in a single open review import (deduped on re-sync)", async () => {
    ...
    const r = await syncFirm(firmId, { trigger: "manual", clientId, client, userId: "u1" });
    ...
    const r2 = await syncFirm(firmId, { trigger: "manual", clientId, client, userId: "u1" });
  ...
  it("throws when the firm has no connected Orion connection", async () => {
    const firmId = freshFirmId();
    await expect(syncFirm(firmId, { trigger: "manual", userId: "u1" })).rejects.toThrow();
  });
});
