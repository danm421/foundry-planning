...
export const engineFlowRules: Rule = (input) => {
  ...
  if (ira != null && ira > 1_000) {
    ...
    if (p < 0.5 * ira) suggestions.push({ id, section: "income", kind: "review", status: "open",
      headline: `${money(ira)} came out of IRAs in ${taxYear}; the plan draws ${money(p)} from retirement accounts in ${planYear}.`,
      meaning: "If that money was spent, living expenses are understated — see the Spending card. If it was reinvested, a transfer to a taxable account is missing. Either way, the withdrawal strategy on Techniques decides which accounts the plan draws first.",
  ...
  if (inv != null) {
    ...
    if (inv > 2 * p && inv - p > 2_000) suggestions.push({ id, section: "income", kind: "review", status: "open",
  ...
  if (gain != null && gain > 5_000) {
    ...
    if (p < 0.25 * gain) suggestions.push({ id, section: "income", kind: "review", status: "open",
  ...
  if (other != null && other > 5_000) {
    ...
    if (p < 0.5 * other) suggestions.push({ id, section: "income", kind: "info", status: "open", headline: `The return shows ${money(other)} of other income the plan does not carry.`, meaning: "Unemployment and one-off income are usually not recurring; add a row only if this repeats.", ...fig, delta: makeDelta(other, p) });
