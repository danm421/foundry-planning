...
function AccountBeneficiaryEditor({
    ...
    return (
      <div className="mt-3">
        <div className="flex items-center justify-between">
          ...
          <span className={sumOk ? "text-xs text-good" : "text-xs text-warn"}>
            sum: {sum.toFixed(2)}%
          </span>
        </div>
          ...
          {tierRows.map((r) => (
            <li key={r.id} className="flex items-center gap-2">
              ...
              <input
                ...
                className={`${inputBaseClassName} w-24 text-right`}
              />
  ...
  return (
    <div className="mt-3 border-t border-hair pt-3">
      ...
      {saveError && <div className="mt-2 text-sm text-crit">{saveError}</div>}
      <button
        type="button"
        disabled={saving}
        onClick={save}
