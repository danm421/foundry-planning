...
import { StartPlanningPrompt } from "@/components/start-planning-prompt";
...
export function CrmHouseholdForm({ mode }: CrmHouseholdFormProps) {
  ...
  // Set once the household exists and there's no returnTo — the form stops
  // navigating and offers the planning start paths over the page instead.
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // Otherwise stay put and offer the planning start paths. `submitting`
      // deliberately stays true for as long as the prompt is open, so the
      // form behind it can't be resubmitted into a duplicate household.
      setCreated({ id: household.id, name: household.name ?? name.trim() });
    } catch (err) {
  ...
  return (
    <>
      ...
      {created && <StartPlanningPrompt household={created} />}
    </>
  );
}
