// src/components/reports/widget-inspectors/risk-severity-bar.tsx
//
// Inspector body for the riskSeverityBar widget. A·Content (title),
// B·Rows (editable list with area + severity dropdown, plus add/remove),
// D·Notes.
//
// Mirrors the row-editor pattern from the riskTable inspector — the
// severity bar widget intentionally uses the same row shape (minus the
// `description` column) so authors can keep the two widgets visually in
// sync. Future-work: shared editing of the source list across both
// widgets.

import type { WidgetInspectorProps } from "@/lib/reports/widget-registry";
import type {
  RiskSeverity,
  RiskSeverityBarProps,
...
export function RiskSeverityBarInspector({
  props,
  onChange,
}: WidgetInspectorProps<"riskSeverityBar">) {
  function updateRow(
    index: number,
    patch: Partial<RiskSeverityBarProps["rows"][number]>,
  ) {
  ...
  function addRow() {
    onChange({
      ...props,
      rows: [...props.rows, { area: "", severity: "medium" }],
    });
  }
