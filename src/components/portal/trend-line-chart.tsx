// src/components/portal/trend-line-chart.tsx
"use client";
...
export function TrendLineChart({
  series, asOfDate, heading, seriesLabel = "Value", initialWindow = "1Y",
}: {
  series: TrendPoint[];
  asOfDate: string;
  heading?: string;
  ...
  const [win, setWin] = useState<TrendWindow>(initialWindow);
  ...
  return (
    <section className="space-y-3">
      <div className={`flex items-center ${heading ? "justify-between" : "justify-end"}`}>
        {heading && (
          <h3 className="text-sm font-medium text-ink-2">{heading}</h3>
        )}
