...
import { buildSolverIncomeChartData } from "../solver-income-chart";

describe("buildSolverIncomeChartData", () => {
  it("stacks one bar per drawn source, plus a Living Expenses line", () => {
    const data = buildSolverIncomeChartData([
      row({
        year: 2026,
  ...
  it("gives each source a distinct color and re-resolves them per theme", () => {
    ...
    const dark = buildSolverIncomeChartData(rows, "dark").datasets.filter((d) => d.type === "bar");
    const light = buildSolverIncomeChartData(rows, "light").datasets.filter((d) => d.type === "bar");
  ...
  it("renders a bare line when nothing is withdrawn in any year", () => {
    const data = buildSolverIncomeChartData([row({ livingExpenses: 50_000 })]);
