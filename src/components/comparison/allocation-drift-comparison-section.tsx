...
function pctSeries(years: ProjectionYear[], key: keyof ProjectionYear["portfolioAssets"]): number[] {
  return years.map((y) => {
...
function PlanChart({ plan, yearRange, index }: { plan: ComparisonPlan; yearRange: YearRange | null; index: number }) {
  ...
  const options = useMemo(
    () => ({
      ...
      plugins: {
        ...
        tooltip: { backgroundColor: "#1f2937", callbacks: { label: (ctx: { dataset: { label?: string }; parsed: { y: number } }) => `${ctx.dataset.label}: ${ctx.parsed.y.toFixed(1)}%` } },
      },
      scales: {
        x: { stacked: true, ticks: { color: "#9ca3af" }, grid: { color: "#1f2937" } },
        y: { stacked: true, max: 100, ticks: { color: "#9ca3af", callback: (v: number | string) => `${v}%` }, grid: { color: "#1f2937" } },
      },
    }),
    [],
  );
