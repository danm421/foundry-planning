...
describe("CardList — expanded mode", () => {
  // Property is the last caller of this shape; it must keep working untouched.
  function ExpandedHost({ initial = [] }: { initial?: Thing[] }) {
    const [items, setItems] = useState<Thing[]>(initial);
    return (
      <CardList
        heading="Things"
        addLabel="Add thing"
        emptyMessage="No things added yet."
        items={items}
        onAdd={() => setItems((cur) => [...cur, { name: `Thing ${cur.length + 1}`, amount: 0 }])}
        onRemove={(index) => setItems((cur) => cur.filter((_, i) => i !== index))}
        renderItem={(thing, i) => (
  ...
  it("renders the heading and every card expanded at once", () => {
    render(<ExpandedHost initial={A_B_C} />);

    expect(screen.getByRole("heading", { name: "Things" })).toBeInTheDocument();
    expect(isOpen("Alpha")).toBe(true);
    expect(isOpen("Beta")).toBe(true);
    expect(isOpen("Gamma")).toBe(true);
    // No collapse affordances in this mode.
    expect(screen.queryByRole("button", { name: /^done$/i })).not.toBeInTheDocument();
  });

  it("shows the plain empty message and removes by index", () => {
    render(<ExpandedHost initial={A_B_C} />);
    expect(screen.queryByText("No things added yet.")).not.toBeInTheDocument();
