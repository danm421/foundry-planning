...
describe("buildPortfolioDatasets", () => {
  ...
  it("each series reads the matching *Total field", () => {
    ...
    const y = makeYear({
      year: 2026,
      portfolioAssets: {
        taxable: {}, cash: {}, retirement: {}, annuity: {}, realEstate: {}, business: {}, lifeInsurance: {}, stockOptions: {},
    ...
    expect(series.find((s) => s.label === "Annuity")!.valueFor(y)).toBe(40_000);
  ...
  it("H1: portfolio chart segments sum to portfolioAssets.liquidTotal", () => {
    const y = {
      portfolioAssets: {
        cashTotal: 10, taxableTotal: 20, retirementTotal: 30, annuityTotal: 3,
        lifeInsuranceTotal: 5,
        accessibleTrustAssetsTotal: 7, realEstateTotal: 100, businessTotal: 50,
        trustsAndBusinessesTotal: 40, liquidTotal: 75, total: 218,
      },
    } as never;
