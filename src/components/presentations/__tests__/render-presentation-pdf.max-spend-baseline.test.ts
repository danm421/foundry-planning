...
// Monte Carlo fires for `retirementComparison` (via `plannerFlagsFor` in
// export-page-sets); stub it so this test measures the max-spend wiring alone.
...
vi.mock("@/components/presentations/registry", () => {
  ...
  // and BOTH hooks read the one helper — the route drives the solve off
  // `maxSpendRefs`, so a mock that omits it would run no solve at all and
  ...
  return {
    PRESENTATION_PAGES: {
      retirementComparison: {
        ...
        requiredScenarioRefs: refs,
        maxSpendRefs: (o: Opts) =>
          o.maxSpend.show ? { refs: refs(o), targetPoS: o.maxSpend.targetConfidence } : null,
      },
    },
  };
});
