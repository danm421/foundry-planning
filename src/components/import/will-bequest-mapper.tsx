...
// didn't extract.
const SELECT_EMPTY = `${selectClassName} bg-warn/10 border-warn/40`;
...
export default function WillBequestMapper({
  ...
  const containerTone = bequest.discarded
    ? "border-hair bg-card-2/30 opacity-50"
    : resolved
        ...
        <div className="grid grid-cols-4 gap-2">
          <div className="col-span-2">
            ...
            <select
              ...
              className={bequest.kind ? selectClassName : SELECT_EMPTY}
            >
          ...
          <div className="col-span-2">
            <label className={fieldLabelClassName}>Recipient</label>
            <select
              value={findEncodedRecipient(bequest)}
              onChange={(e) =>
                onChange(applyRecipientSelection(bequest, e.target.value))
              }
              className={bequest.recipientKind ? selectClassName : SELECT_EMPTY}
            >
          ...
          <div>
            <label className={fieldLabelClassName}>Display name</label>
            <input
              value={bequest.name ?? ""}
              onChange={(e) =>
