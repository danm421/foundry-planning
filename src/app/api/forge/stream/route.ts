...
export async function POST(req: Request): Promise<Response> {
  ...
  // NO client access gate — this is the global (clientless) route.
  let firmId: string;
  let userId: string;
  let firmName: string;
  ...
  // 5. Rate limit (fail-closed; exceeded→429 else→503).
  ...
  let cid: string;
  let authContext: ForgeGlobalAuthContext;
  ...
  try {
    ...
    if (conversationId) {
      ...
      conversationId = await createConversation({
        ...
        // clientId intentionally omitted → null (global/clientless conversation)
      });
    }
    cid = conversationId;

    authContext = { userId, firmId };
    ...
    await recordAudit({
      ...
      firmId,
      actorId: userId,
      metadata: { mode: "global", currentPage: body.currentPage ?? null },
    });

    systemPrompt = () => buildGlobalSystemPrompt({ firmName, advisorName, todayISO });
  } catch {
    return json(500, { error: "Internal server error." });
  }

  const graph = buildGraph(authContext, getCheckpointer(), cid, systemPrompt);
  // maybeLangfuseHandler expects ForgeAuthContext; global ctx satisfies the
  // userId/firmId fields it reads for tracing — clientId/scenarioId will be
  // undefined in the trace metadata, which is correct for a clientless turn.
  const langfuse = maybeLangfuseHandler(authContext as ForgeAuthContext, cid);
  const encoder = new TextEncoder();

  // SSE body: COPIED VERBATIM from the client stream route
  // (src/app/api/clients/[id]/forge/stream/route.ts).
  // The only difference in this block: the meeting_review branch is dropped
  // (global Plan 1 has no meeting tool), so only parseApprovalInterrupt is kept.
    ...
    async start(controller) {
      ...
      try {
        const events = graph.streamEvents(
          { messages: [new HumanMessage(message)], authContext, verifyAttempts: 0 },
          {
            version: "v2",
        ...
        if (!closed) {
          ...
          // NOTE: meeting_review branch is NOT present here — global Plan 1 has
          // no meeting tool, so only parseApprovalInterrupt is needed.
