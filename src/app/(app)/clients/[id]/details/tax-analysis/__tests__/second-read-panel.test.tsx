...
describe("SecondReadPanel", () => {
  it("offers to run the read when none exists", () => {
    render(<SecondReadPanel secondRead={null} stale={false} busy={false} onGenerate={noop} onDismiss={noop} />);
  ...
  it("labels every item AI-read and unverified", () => {
    render(<SecondReadPanel secondRead={read()} stale={false} busy={false} onGenerate={noop} onDismiss={noop} />);
  ...
  it("hides dismissed items", () => {
    ...
    render(<SecondReadPanel secondRead={r} stale={false} busy={false} onGenerate={noop} onDismiss={noop} />);
  ...
  it("calls onDismiss with the item id", async () => {
    ...
    render(<SecondReadPanel secondRead={read()} stale={false} busy={false} onGenerate={noop} onDismiss={onDismiss} />);
  ...
  it("warns that regenerating clears dismissals — the stated behaviour, surfaced", () => {
    render(<SecondReadPanel secondRead={read()} stale={true} busy={false} onGenerate={noop} onDismiss={noop} />);
  ...
  it("says nothing was found rather than rendering an empty box", () => {
    render(
      <SecondReadPanel secondRead={read({ items: [] })} stale={false} busy={false} onGenerate={noop} onDismiss={noop} />,
    );
  ...
  it("surfaces documents that could not be read — 'nothing found' must not mean 'nothing looked at'", () => {
    render(
      <SecondReadPanel
        ...
        stale={false} busy={false} onGenerate={noop} onDismiss={noop}
      />,
    );
  ...
  it("disables both actions while busy", () => {
    render(<SecondReadPanel secondRead={read()} stale={false} busy={true} onGenerate={noop} onDismiss={noop} />);
