...
interface Props {
  ...
  /** Overrides the caption above the number. The Education tab measures one
   *  goal rather than the whole plan, so it reads "Goal Confidence" there. */
  label?: string;
}

export function SolverPosGauge({ state, successPct, onRegenerate, solveActive, baselineSuccessPct, label = "Plan Confidence" }: Props) {
  let display: string;
  ...
  return (
    <div className="relative">
      <div className={contentDim}>
        ...
        >
          {state === "computing" ? (
            <span role="status" aria-label={`Calculating ${label.toLowerCase()}`}>
      ...
      {showOverlay ? (
        <div className="absolute inset-0 flex items-center justify-center">
          <button
            ...
            aria-label={`Recalculate ${label.toLowerCase()}`}
