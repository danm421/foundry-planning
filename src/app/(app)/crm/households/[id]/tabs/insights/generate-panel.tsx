...
function Section({ title, body }: { title: string; body: string }) {
  ...
  return (
    <section className="rounded-[var(--radius)] border border-hair-2 bg-card p-5">
      ...
      <div className="whitespace-pre-wrap text-[13px] leading-relaxed text-ink-2">{body}</div>
    </section>
  );
}
          ...
          return (
            <li key={a.signalId} className="flex gap-3">
              ...
              <div className="flex min-w-0 flex-col gap-0.5">
                ...
                <span className="text-[13.5px] font-semibold text-ink">{a.recommendation}</span>
                <span className="text-[13px] text-ink-2">{a.why}</span>
              </div>
            </li>
          );
        })}
 ...
 *  `household-360.tsx` — the shape must not change without updating that
 *  call site. */
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          ...
          onClick={run}
          disabled={loading}
          className="rounded-[var(--radius-sm)] border border-hair-2 px-3 py-1.5 font-mono text-[10.5px] font-semibold uppercase tracking-[0.12em] text-ink transition-colors duration-150 hover:border-accent hover:text-accent disabled:opacity-50 disabled:hover:border-hair-2 disabled:hover:text-ink"
        >
      ...
      {sections && (
        <>
          {sections.headline && (
            <p className="font-display text-[clamp(20px,2.4vw,28px)] leading-[1.08] tracking-[-0.03em] text-ink [text-wrap:pretty]">
              {sections.headline}
            </p>
          )}
